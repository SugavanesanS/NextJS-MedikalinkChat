...
import { collection, doc, FirestoreError, getDoc, getDocs, getFirestore, Timestamp, increment, limit, onSnapshot, orderBy, query, setDoc, startAfter, updateDoc, where, writeBatch } from "firebase/firestore"
...
import { ScreenLoader } from "../services/CommonFunction"
...
import { assigneeSearchUserType, AttachmentData, ChatMessageFireBase, ChatUserProfile, DiscussionUser, FBDiscussionList, FBParticipants, FileRes, Files, intAdminType, profileImage, VarChar } from "../types/data"
    ...
    useEffect(() => {
        ...
        const subscription = onSnapshot(
            ...
            (snapshot) => {
                ...
                console.warn('Firebase permission error - check Firestore rules use Discussion (capital D):', error.code)
            }
        );
...
export const useGetChatUserList = () => {
    ...
    const dispatch = useDispatch()
    const chatUserList = useGet<'/v1/discussion/participants-list'>({ endpoint: '/v1/discussion/participants-list' })
...
export const useChatDiscussionHook = (): ChatDiscussionReturnType => {
    ...
    const blockedUsers = useMemo(
        () => Object.values(participants.participants)
            .filter(p => p.blocked_flag == true && p.fbUserId == fbUserId)
    ...
    const loadMore =
        ...
        useCallback(() => {
            ...
            //   console.log("blocked flage ------load data messages 222", participants.participants)
    ...
    useEffect(() => {
        if (discussionId) {
            ...
            //     console.log("blocked flage ------load data messages 222", participants.participants, blockedUsers)
            if (Object.keys(participants.participants).length <= 0) {
                return
            }
            const blockedDate = blockedUsers[0]?.blocked_date

            let q;
            let ConfigDate = Date.now()
            // console.log("blocked date ---", blockedDate, ConfigDate)
            if (blockedDate) {
                console.log("blocked date logic")
                let targetDate: number = (Timestamp.fromDate(new Date(blockedDate)).toMillis()); // Example target date
                console.log("target date ", targetDate)
                q = query(collection(db, `Discussion/${discussionId}/messages`), where("sentAt", "<=", targetDate)); //orderBy("sentAt", "asc")
                ConfigDate = Date.now()
            }
            else {
                console.log("general logic ")
                q = query(collection(db, `Discussion/${discussionId}/messages`));
                ConfigDate = Date.now()
    ...
    }, [discussionId, Object.keys(participants.participants).length])
...
export const usePostCall = () => {
    ...
    const open = useCallback((discussionId: number, assigneeSearch: assigneeSearchUserType = 'all', reset: boolean = false) => window.postMessage({
        ...
        const func = (event: { data: { type: keyof typeof ChatPostMessageKey, data: any, fileData?: any }, origin: string }) => {
            ...
            switch (event.data.type) {
                case ChatPostMessageKey.OPEN_CHAT:
                    ...
                    let assigneeSearchUserType = event.data.data.assigneeSearch
                    ...
                    dispatch(DiscussionActions.setAssigneeSearchUserType(assigneeSearchUserType))
                ...
                case ChatPostMessageKey.OPEN_CHAT_CREATE:
                    dispatch(DiscussionActions.setAssigneeSearchUserType('all'))
    ...
    const CreateNewDiscussion = useCallback(
        async (usersData: Record<string, ChatUserProfile>,
            ...
            const discussionData = {
                ...
                participants,
                unreadCount,
                discussionMarkStatus: discussionMarkStatus
            };
...
export const useOtherFunctionality = () => {
    ...
    const updateDiscussionMarkStatus = useCallback((state: Number) => {
        ...
        console.log("discussion id ----------", discussionId, state)
        const patch = writeBatch(db);
        patch.update(doc(db, `Discussion/${discussionId}`), {
...
export const useChatFileUpload = () => {
    ...
    const upload = useCallback(
        async ({ file, discussion_id, type, participants, discussionGuid, audio_duration }: { discussionGuid: VarChar, file: File | Files, discussion_id?: string, type: '1' | '2', participants?: Record<number, FBParticipants> | undefined, audio_duration?: number }) => new Promise((resolve, reject) => {
            ScreenLoader(true);
            ...
            if (type == '1') {
                ...
                if ((file as File).type.includes('audio')) {
                    filetype = 'mp3'
                } else {
